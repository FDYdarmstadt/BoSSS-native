...
OpenBLAS, FLAME \cite{GotoVanDeGeijn08}. or vendor-provide BLAS.
ParU relies heavily on nested parallelism, by making parallel calls to
the BLAS, each of which can themselves be parallel.  This is supported by the
Intel MKL BLAS library with an Intel-specific method
(\verb'mkl_set_num_threads_local') to tell the BLAS how many threads to use.
As a result, tor best performance, the Intel MKL BLAS is required.
...
FIXME: discuss \verb'OMP_MAX_ACTIVE_LEVELS=2'
